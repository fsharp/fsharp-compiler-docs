...
type internal IncrementalBuilder = 
      ...
      member Step : TimeStampCache * ct: CancellationToken -> bool
...
module internal IncrementalBuild =
    ...
    val Eval : TimeStampCache -> (PartialBuild -> unit) -> CancellationToken -> INode -> PartialBuild -> PartialBuild

    /// Evaluate a build for a vector up to a limit. Only required for unit testing.
    val EvalUpTo : TimeStampCache -> (PartialBuild -> unit) -> CancellationToken -> INode * int -> PartialBuild -> PartialBuild

    /// Do one step in the build. Only required for unit testing.
    val Step : TimeStampCache -> (PartialBuild -> unit) -> CancellationToken -> Target -> PartialBuild -> PartialBuild option
